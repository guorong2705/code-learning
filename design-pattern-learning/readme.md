...
### 要点
在桥接模式中，有两个独立的维度：抽象部分和实现部分，抽象部分定义了高层接口，通常包含一个实现部分的引用；实现部分定义了底层的实现接口。桥接模式使用组合关系将这两个维度连接起来，使得它们可以独立变化，不会紧密耦合在一起。

### 模式结构
- 抽象部分（Abstraction）：定义高层接口，通常包含一个实现部分的引用。它可以是一个抽象类或接口。
- 扩展抽象部分（Refined Abstraction）：扩展抽象部分，增加额外的功能。它继承了抽象部分，并可以实现一些高层接口之外的功能。
- 实现部分（Implementor）：定义底层的实现接口，通常也是一个抽象类或接口。
...
## 装饰器模式 (decorator)
### 意图
装饰者模式`(Decorator Pattern)`是一种结构型设计模式，它允许你动态地将新功能添加到对象中，同时保持原有对象的结构不变。装饰者模式通过使用组合关系来实现这种功能扩展，而不是通过继承关系，从而避免了类爆炸问题。
...
- 当真实主题的创建和销毁非常消耗资源时，可以使用代理模式进行懒加载（延迟加载）。
...
责任链模式的主要思想是将请求和处理分离，避免了请求发送者和接收者之间的紧耦合关系，同时可以动态地组合处理链。
...
### 意图：
定义一个操作中算法的骨架，而将一些步骤延迟到子类中。 模板方法使得子类可以不改变一个算法的结构，即可重定义该算法的某些特定步骤。
